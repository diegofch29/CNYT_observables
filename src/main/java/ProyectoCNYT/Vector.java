...
public class Vector {
    ...
    public Numero_complejo InnerProduct(Vector v){
        ...
        Vector v1=this.Conjugado();
        //v.print();
        for (int i=0;i<v.n;i++){
            result=result.sumar(v1.Elementos[i].Multiplicar(v.Elementos[i]));
        }
        return result;
    }
    ...
    public double Distancia(Vector v){
        Vector resp =this.Restar(v);
        return Math.sqrt(resp.Norma());
    }
    ...
    public double Probabilidad(int pos){
        double resp=Math.pow(this.getElemento(pos).Modulo(),2)/Math.pow(this.modulo(),2);
        return resp;
    }
    
    public Vector normalizar(){
        return this.Multiplicacion_Escalar(new Numero_complejo(1/this.Norma(),0));
    }
    
    public Numero_complejo amplitud(Vector V){
        V.InnerProduct(this).print();
        return V.InnerProduct(this).Dividir(new Numero_complejo(this.Norma()*V.Norma(),0));
    }
